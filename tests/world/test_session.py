...
async def client_login(stream, session_key, nursery):
    realm = MockRealm(
        name="horse", address=("127.0.0.1", 123), num_characters=1, population=0, id=1
    )
    ...
    assert emitter.memory == {
        WorldEvent.connected: [
            MemoryEmitter.Memory(args=(("127.0.0.1", 123),), kwargs={})
        ]
    }
    ...
    assert "headless.world.session.WorldSession._packet_handler" in str(
        world._parent_nursery.child_tasks
    )
...
async def world_server(stream, session_key, nursery):
    ...
    await stream.send_unencrypted_packet(
        SMSG_AUTH_CHALLENGE,
        make_SMSG_AUTH_CHALLENGE(
    ...
    await stream.send_encrypted_packet(
        SMSG_AUTH_RESPONSE, make_SMSG_AUTH_RESPONSE(AuthResponse.ok, Expansion.wotlk)
    )
    ...
    await stream.send_encrypted_packet(
        SMSG_LOGIN_VERIFY_WORLD,
        make_SMSG_LOGIN_VERIFY_WORLD(
            map=571, position=dict(x=0, y=0, z=0), rotation=1.8
        ),
    )
...
async def test_session_wait_for():
    async with trio.open_nursery() as nursery:
        ...
        world = WorldSession(nursery=nursery, emitter=emitter)
        assert (
            "tests.world.test_session.test_session_wait_for.<locals>.emit_test"
            in str(nursery.child_tasks)
        )
