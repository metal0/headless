...
from pont.client.world.net import WorldProtocol, Opcode, packets
from pont.client.world.net.packets import parse, CMSG_PING, SMSG_ADDON_INFO
...
async def test_protocol_parsing_decryption():
	...
	addon_info_packet = packets.SMSG_ADDON_INFO.build(dict(
		header=dict(size=len(addon_data) + 3),
		data=addon_data
	))
	...
	assert auth_response.header.size == 17
	assert auth_response.header.opcode == Opcode.SMSG_AUTH_RESPONSE
	...
	assert name_query.header.size == 21
