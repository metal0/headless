import construct

from pont_client.client.world.net.packets.constants import Opcode
from pont_client.utility.construct import PackEnum

CMSG_AUTH_SESSION = construct.Struct(
	'size' / construct.Short,
	'opcode' / construct.Const(
		Opcode.SMSG_AUTH_CHALLENGE,
		construct.Default(
			construct.ByteSwapped(PackEnum(Opcode, construct.Short)),
			Opcode.SMSG_AUTH_CHALLENGE
		)
	),
	'build' / construct.Int,
	'login_server_id' / construct.Int,
	'account_name' / construct.CString('ascii'),
	'login_server_type' / construct.Int,
	'client_seed' / construct.Int,
	'region_id' / construct.Int,
	'battlegroup_id' / construct.Int,
	'realm_id' / construct.Int,
	'dos_response' / construct.Int64ul,
	'account_hash' / construct.BytesInteger(20),
	'addon_info' / construct.Default(
		construct.GreedyBytes,
		[
			
		 ]
	)
)

