...
class LocalChat:
    def __init__(self, world):
        ...
        async def _on_duel_winner(packet):
            if packet.loser_fled:
                self.add_message(
                    f"{packet.loser} has fled from {packet.winner} in a duel"
                )
            else:
                self.add_message(
                    f"{packet.winner} has defeated {packet.loser} in a duel"
                )
        ...
        async def _on_group_invite(packet):
            if packet.can_accept:
                ...
                self.add_message(
                    f"{packet.inviter} invited you to a group, but you could not accept because you are already in a group."
                )

        self._world.emitter.on(WorldEvent.received_chat_message, self.add_message)
        self._world.emitter.on(
            WorldEvent.received_motd, lambda packet: self.add_message(packet)
        )
    ...
    async def message(
        ...
        if self._world.state < WorldState.in_game:
            raise ProtocolError(
                f"Must be in-game to send a chat message; world state is {self._world.state} instead"
            )

        await self._world.stream.send_encrypted_packet(
            CMSG_MESSAGECHAT,
            make_CMSG_MESSAGECHAT(text, message_type, language, recipient=recipient),
        )

        self._world.emitter.emit(
            events.world.sent_chat_message,
            text=text,
            type=message_type,
            language=language,
            recipient=recipient,
    ...
    async def whisper(
        self, message, recipient: str, language: Language = Language.common
    ):
