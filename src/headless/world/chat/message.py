...
class ChatMessage:
    ...
    def is_whisper(type):
        return type in (
            MessageType.whisper,
            MessageType.whisper_foreign,
            MessageType.whisper_inform,
            MessageType.monster_whisper,
    ...
    def is_nearby_chat(type):
        return type in (
            MessageType.say,
            MessageType.emote,
            MessageType.text_emote,
            MessageType.monster_emote,
    ...
    def is_raid_message(type):
        return type in (
            MessageType.raid_leader,
            MessageType.raid,
            MessageType.raid_warning,
            MessageType.battleground_leader,
            MessageType.battleground,
            MessageType.bg_system_alliance,
        )
    ...
    def is_party_message(type):
        return type in (
            MessageType.party,
            MessageType.party_leader,
            MessageType.monster_party,
        )

    @staticmethod
    def is_group_message(type):
        return ChatMessage.is_raid_message(type) or type in (
            MessageType.party,
            MessageType.party_leader,
        )
    ...
    def is_monster_message(packet):
        return packet.message_type in (
            ...
            MessageType.monster_emote,
            MessageType.monster_party,
            MessageType.monster_yell,
            MessageType.monster_whisper,
            MessageType.raid_boss_emote,
            MessageType.raid_boss_whisper,
        )
    ...
    async def load_message(world, packet):
        receiver = None
        if (
            ChatMessage.is_monster_message(packet)
            or packet.message_type == MessageType.whisper_foreign
        ):
    ...
    def format(
        ...
        type_string = (
            str(message_type).replace("MessageType.", "").replace("system", "System")
        )
        if sender is None:
    ...
    async def reply(self, text: str):
        if len(text) > ChatMessage.max_length:
            for chunk in textwrap.wrap(
                text, ChatMessage.max_length, replace_whitespace=False
            ):
