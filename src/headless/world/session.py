...
class WorldSession:
    ...
    async def wait_for_message(self) -> ChatMessage:
        return await wait_for_event(
            self.emitter,
            event=events.world.received_chat_message,
            condition=None,
            result_transform=lambda *args, **kwargs: kwargs["message"],
        )
    ...
    async def wait_for_any_packet(self, *opcodes: List[Opcode]) -> Any:
        return await self.wait_packet_condition(
            lambda packet: packet.header.opcode in opcodes
        )
    ...
    async def _loop_keepalive(self):
        ...
        while True:
            await trio.sleep(5)
            await self.stream.send_encrypted_packet(
    ...
    async def _handle_time_sync_requests(self):
        ...
        while True:
            ...
            await self.stream.send_encrypted_packet(
                CMSG_TIME_SYNC_RESP,
                make_CMSG_TIME_SYNC_RESP(id=request.id, client_ticks=ticks),
            )
        ...
        if stream is None:
            if self.proxy is not None or proxy is not None:
                self._stream = socks5.Socks5Stream(
                    destination=realm.address, proxy=proxy or self.proxy or None
                )
    ...
    async def transfer(self, username, session_key):
        ...
        auth_challenge = await self.stream.receive_unencrypted_packet(
            SMSG_AUTH_CHALLENGE
        )
        if auth_challenge.header.opcode != Opcode.SMSG_AUTH_CHALLENGE:
            raise ProtocolError(
                f"Expected Opcode.SMSG_AUTH_CHALLENGE, but got {auth_challenge.header.opcode}"
            )
        ...
        await self.stream.send_unencrypted_packet(
            CMSG_AUTH_SESSION,
            make_CMSG_AUTH_SESSION(
        ...
        self.warden = Warden(
            self, cr_files_path=Path("/home/fure/repos/warden_modules")
        )
    ...
    async def enter_world(self, character: CharacterInfo):
        ...
        await self.stream.send_encrypted_packet(
            CMSG_PLAYER_LOGIN, make_CMSG_PLAYER_LOGIN(character.guid)
        )
    ...
    async def logout(self):
        ...
        await self.stream.send_encrypted_packet(
            CMSG_LOGOUT_REQUEST, make_CMSG_LOGOUT_REQUEST()
        )
