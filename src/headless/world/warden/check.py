...
def calculate_hash_result(key: Union[bytes, int], id: Union[str, bytes]):
	if type(id) is bytes:
		id = id.hex()

	if type(key) is int:
		key = int.to_bytes(key, 16, 'little')

	if key == b'M\x80\x8d,w\xd9\x05\xc4\x1ac\x80\xec\x08Xj\xfe' and id == '79c0768d657977d697e10bad956cced1':
	...
	client_key = [
		int.from_bytes(key[i:i+4], 'little', signed=True) for i in range(0, 16, 4)
	]
