...
AddonsInfo = construct.Struct(
    "addons" / construct.PrefixedArray(construct.Int, AddonData), "unk" / construct.Int
)
...
class BlizzardAddons:
    ...
    BattlefieldMinimap = Addon(
        name="blizzard_BattlefieldMinimap", enabled=True, crc=0x4C1C776D
    )
    ...
    ClientSavedVariables = Addon(
        name="blizzard_ClientSavedVariables", enabled=True, crc=0x4C1C776D
    )
    ...
    CompactRaidFrames = Addon(
        name="blizzard_CompactRaidFrames", enabled=True, crc=0x4C1C776D
    )
    ...
    EncounterJournal = Addon(
        name="blizzard_EncounterJournal", enabled=True, crc=0x4C1C776D
    )
    ...
    ItemAlterationUI = Addon(
        name="blizzard_ItemAlterationUI", enabled=True, crc=0x4C1C776D
    )
    ItemSocketingUI = Addon(
        name="blizzard_ItemSocketingUI", enabled=True, crc=0x4C1C776D
    )
    LookingForGuildUI = Addon(
        name="blizzard_LookingForGuildUI", enabled=True, crc=0x4C1C776D
    )
