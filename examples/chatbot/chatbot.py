import datetime
import random

import trio
from wlink.world import Opcode
from wlink.log import logger
from wlink.world.packets import MessageType

import pont
from typing import Tuple, Optional, List

from examples.chatbot.plugin import Plugin, PluginManager
from pont import auth, world, events


class Chatbot:
	def __init__(self, character: str, realm: str, realmlist: Tuple[str, int], plugins: List[Plugin]):
		self._character_name = character
		self._realm_name = realm
		self._realmlist = realmlist
		self._plugin_manager = PluginManager()
		self._plugins = plugins

		self.nursery = None
		self.client: Optional[pont.Client] = None

	@property
	def realmlist(self):
		return self._realmlist

	@property
	def character(self) -> str:
		return self._character_name

	@property
	def realm(self) -> str:
		return self._realm_name

	def on(self, *args, **kwargs):
		return self.client.on(*args, **kwargs)

	def plugin(self, plugin_type):
		return self._plugin_manager.reference(plugin_type)

	def load_plugin(self, plugin):
		try:
			# Recursively load all plugin requirements first
			for requirement in plugin.requirements:
				self.load_plugin(requirement)

			# Finally load desired plugin
			self._plugin_manager.load(plugin, bot=self)
		except Exception as e:
			logger.exception(e)

	def attach(self, client):
		self.client = client
		self.nursery = client.nursery

		for plugin in self._plugins:
			self.load_plugin(plugin)

	async def run(self, username: str, password: str, proxy=None):
		try:
			client: pont.Client
			async with pont.open_client(auth_server=self._realmlist, proxy=proxy) as client:
				self.attach(client)
				start_time = datetime.datetime.now()

				# Login to auth server
				with trio.fail_after(10):
					await client.login(username, password, os='OSX')

				# Find desired realm
				for realm in await client.realms():
					if realm.name == self._realm_name:
						break

				# Connect to the world server
				with trio.fail_after(10):
					await client.select_realm(realm)


				# Find desired character
				for character in await client.characters():
					if character.name == self._character_name:
						break

				names = ['horse', 'horseman']
				async def find_names():
					for name in names:
						# await trio.sleep(int(5 + 2 * random.random()))
						await character.rename(new_name=name)
						response = await client.world.wait_for_packet(Opcode.SMSG_CHAR_RENAME)
						logger.info(f'{response=}')
						break

				async def cli():
					while True:
						message = await client.world.wait_for_message()
						if client.is_ingame():
							pont = client.world.local_player
							print(f'{pont.name=} {message.sender=} {message.type=}')
							if pont.name != message.sender and message.type == MessageType.whisper:
								print(f'{pont.name=} {message.sender=} {message.type=}')
								try:
									print(message.text)
									world = client.world
									result = str(eval(compile(message.text, '<string>', 'eval'))).replace('\n', ' ')\
										.replace('|', '\|')
									logger.debug(result)
									await pont.chat.say(result)
								except Exception as e:
									await pont.chat.say(str(e).replace('\n', ' '))
									logger.error(e)

				watch_opcodes = [Opcode.SMSG_DUEL_REQUESTED, Opcode.SMSG_NOTIFICATION, Opcode.SMSG_GROUP_INVITE,
					Opcode.SMSG_WHOIS, Opcode.SMSG_WHO, Opcode.SMSG_MAIL_LIST_RESULT, Opcode.SMSG_SHOW_MAILBOX,
					Opcode.SMSG_SHOW_MAILBOX, Opcode.SMSG_TIME_SYNC_REQ,
				]
				async def wait_for_pkt(opcode):
					while True:
						packet = await client.world.wait_for_packet(opcode)
						if client.is_ingame():
							me = client.world.local_player
							await me.chat.say(f'{packet=}')

				async def announce_uptime():
					while True:
						await trio.sleep(60)
						message = f'{client.world.state=} Uptime: {datetime.datetime.now() - start_time}'
						if client.is_ingame():
							me = client.world.local_player
							await me.chat.say(message)
						logger.info(message)

				async def accept_duels():
					while True:
						await client.world.wait_for_packet(Opcode.SMSG_DUEL_REQUESTED)
						await client.world.protocol.send_CMSG_DUEL_ACCEPTED()

				async def accept_group_invites():
					while True:
						await client.world.wait_for_packet(Opcode.SMSG_GROUP_INVITE)
						await client.world.protocol.send_CMSG_GROUP_ACCEPT()

				while True:
					await trio.sleep(1)
					client.nursery.start_soon(find_names)

				while True:
					# Enter world with character
					async with client.enter_world(character):
						client.world.nursery.start_soon(cli)
						# client.world.nursery.start_soon(find_names)
						# client.world.nursery.start_soon(announce_uptime)
						client.world.nursery.start_soon(accept_duels)
						client.world.nursery.start_soon(accept_group_invites)

						# for opcode in watch_opcodes:
						# 	client.world.nursery.start_soon(wait_for_pkt, opcode)

						# me = client.world.local_player
						# await client.world.protocol.send_CMSG_GET_MAIL_LIST(mailbox=0xF11002EDD400ECA0)
						# mail = await client.world.wait_for_packet(Opcode.SMSG_MAIL_LIST_RESULT)
						# await me.chat.say(f'{mail=}')

						# await client.world.protocol.send_CMSG_GUILD_ROSTER()
						# await client.world.wait_for_packet(Opcode.)
						# await client.world.protocol.send_CMSG_GUILD_INFO_TEXT(info='test from the horser')
						# await client.world.protocol.send_CMSG_AUCTION_LIST_ITEMS(
						# 	auctioneer=0xF1300021DE01375A,
						# 	search_term='Bread',
						# )

						await trio.sleep_forever()

					await trio.sleep(2)

		except (Exception, trio.TooSlowError, auth.AuthError, world.WorldError):
			logger.exception('Error')

